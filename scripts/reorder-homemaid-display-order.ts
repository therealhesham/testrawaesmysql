...
async function reorderHomemaidDisplayOrder() {
  try {
    ...
    const allHomemaids = await prisma.homemaid.findMany({
      select: {
        id: true,
        displayOrder: true, // ✅ إضافة displayOrder للتحقق من القيم الحالية
      },
    });
    ...
    const nullDisplayOrders = allHomemaids.filter(h => h.displayOrder === null).length;
    if (nullDisplayOrders > 0) {
      console.log(`⚠️ تم العثور على ${nullDisplayOrders} سجل بقيمة displayOrder = null`);
    }
    ...
    console.log(`📈 سيتم تعيين قيم displayOrder من ${totalRecords} إلى 1`);
    ...
    const displayOrderSet = new Set(
      allDisplayOrders.map((h) => h.displayOrder)
    );
    ...
    const stillHasNull = allDisplayOrders.some(h => h.displayOrder === null);
    if (stillHasNull) {
      console.warn('⚠️ تحذير: لا تزال هناك قيم null في displayOrder');
    } else {
      console.log('✅ التحقق: لا توجد قيم null في displayOrder');
    }
  } catch (error) {
