...
import FormExternalStep2 from './FormExternalStep2';

import { createPortal } from "react-dom";
import { saudiCities } from "./SaudiCityAutocomplete";
import { worldCities } from "./CityAutocomplete";
...
const getWarrantyStatus = (KingdomentryDate: string | null | undefined, externaldeparatureDate: string | null | undefined) => {
  if (!KingdomentryDate) {
    return { statusText: 'غير متوفر', statusType: 'unknown' };
  }

  const entryDate = new Date(KingdomentryDate);
  const departureDate = externaldeparatureDate ? new Date(externaldeparatureDate) : new Date();
...
export default function DepartureExternalList({ onOpenModal, canAdd = true, canEdit = true, canDelete = true }: DepartureExternalListProps) {
  ...
  const handleDropdownToggle = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (isNatDropdownOpen) {
      ...
      const container = document.getElementById("external-table-container");
      if (container) {
  ...
  const executeDelete = async () => {
    ...
    try {
      const res = await fetch("/api/deletedeparature", {
        ...
        body: JSON.stringify({ OrderId: orderToDelete, type: "external" }),
      });
      if (res.ok) {
...
const exportToPDF = async () => {
  ...
  if (searchTerm || nationality || startDate || endDate) {
  ...
  return (
    <section id="departure-list" className="mb-10 w-full px-4">
      <div className="flex justify-between items-center mb-8">
        <h2 className="text-xl font-semibold text-gray-700">جدول المغادرة الخارجية</h2>
        {canAdd && (
          <button
            onClick={onOpenModal}
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden mb-6">
        <div className="overflow-x-auto relative min-h-[450px]" id="external-table-container">
                ...
                >
                  ...
                  {visibleColumns.includes('from') && (
                    <td className="py-4 px-5 text-right vertical-align-top min-w-[150px]">
                      <div className="text-sm font-bold text-teal-950 whitespace-nowrap">
                        {row.externaldeparatureCity || "المملكة العربية السعودية"}
                      </div>
                      {row.externaldeparatureDate && (
                        <div className="text-sm font-semibold text-gray-500 font-mono mt-1 whitespace-nowrap">
                          {new Date(row.externaldeparatureDate).toISOString().split('T')[0]}
                        </div>
                      )}
                      {row.externaldeparatureTime && (
                  ...
                  {visibleColumns.includes('to') && (
                    <td className="py-4 px-5 text-right vertical-align-top min-w-[150px]">
                      ...
                      {row.externalArrivalCityDate && (
                        <div className="text-sm font-semibold text-gray-500 font-mono mt-1 whitespace-nowrap">
                          {new Date(row.externalArrivalCityDate).toISOString().split('T')[0]}
                        </div>
                      )}
                      {row.externalArrivalCityTime && (
                        <div className="text-sm text-gray-400 font-medium font-mono mt-0.5 whitespace-nowrap">
                          {row.externalArrivalCityTime}
                        </div>
                      )}
                  ...
                  {visibleColumns.includes('reason') && (
                    <td className="py-4 px-5 text-sm text-gray-600">{row.externalReason || "-"}</td>
                  )}
                  
                  {visibleColumns.includes('warranty') && (
                    <td className="py-4 px-5 text-sm font-semibold">
                      {(() => {
                        const wStatus = getWarrantyStatus(row.KingdomentryDate, row.externaldeparatureDate);
                  ...
                  {visibleColumns.includes('departureStatus') && (
                    <td className="py-4 px-5 text-sm font-semibold">
                      {(() => {
                        const status = getDepartureStatus(row.externaldeparatureDate, row.externaldeparatureTime);
                        if (status === 'غادرت') {
                          return (
                            <span style={{
                              display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '6px 12px',
      ...
      <AlertModal
        ...
        type={alertType}
        title={alertType === 'success' ? 'نجح الحفظ' : 'خطأ'}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full" dir="rtl">
            ...
            <p className="text-text-muted mb-6">
              هل أنت متأكد من مسح بيانات المغادرة الخارجية لـ <strong>{deleteModalNames?.maid}</strong> الخاصة بعميلكم <strong>{deleteModalNames?.client}</strong>؟
              <br />
