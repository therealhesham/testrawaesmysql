...
const PermissionsManagement = ({ currentUserRoleId, currentUserRoleName }: { currentUserRoleId: number; currentUserRoleName: string }) => {
  ...
  const getPermissionItems = (section: string) => {
    ...
    if (section === 'إدارة الإشعارات') {
      return ['اشعارات الطلب الجديد - موقع وايميل', 'اشعارات الطلب الجديد - sms', 'اشعارات ترحيل القيود المالية - موقع وايميل'];
    }
    // قسم المحاسبة
    if (section === 'إدارة المحاسبة') {
      return ['عرض', 'إضافة', 'تعديل', 'حذف', 'عرض صفحة المراجعة'];
    }
