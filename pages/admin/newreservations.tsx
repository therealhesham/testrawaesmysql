...
// console.log(decoder.idnumber)
    ...
    try {
     ...
     await fetch("../api/newreservations").then(response => response.json())
  .then(json  => {
    ...
    <Layout>
      ...
      <TableContainer>
        <Table>
          ...
          <TableCell>تاريخ اخر تعديل</TableCell>
          ...
          <TableBody>
            {fulldata?.map((e, i) => (
              <TableRow key={i}>
                ...
                <TableCell>
                  <span className="text-md">{  e?.fields["Last Modified"].substring(0,10)}</span>
