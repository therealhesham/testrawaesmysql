...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const profession = await prisma.professions.create({ data: { name, gender: gender || null } });
      return res.status(200).json(profession);
    }

    if (req.method === 'PUT') {
      const { id, name, gender } = req.body;
      const profession = await prisma.professions.update({ where: { id }, data: { name, gender: gender || null } });
      return res.status(200).json(profession);
    }
